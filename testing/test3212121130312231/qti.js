...
// (c) Copyright 2024, Jean-Philippe Rivière. All rights reserved.
// This library was modifyed to use Portable Custom Interaction
...
const QTI = {
  ...
  INTERACTIONS: {
    ...
    portableCustomInteraction:   {setup: setupportableCustomInteraction,},
...
const EXECUTORS = {
  ...
  portableCustomInteraction:           extension,
...
// is also how QTI "extension points", like portableCustomInteraction, are
// mapped to an implementation.
function extension(elem, context) {
  //console.log(elem) //janfix
  let result;
...
// Default extension handler for portableCustomInteraction.  Just does a
// normal transform, emitting the portableCustomInteraction as
// a <div data-qtijs-tag="portableCustomInteraction">, etc.
var config = {default : 1}
function portableCustomInteraction(elem) {
  //console.log(elem)

  if(elem.tagName === "property"){
    let keyConfig = elem.getAttribute("key")
    let valueConfig = $(elem).html()
    config[keyConfig] = valueConfig;
  }

  //console.log(config)

  if (elem.tagName==="portableCustomInteraction"){
    
    let src = elem.getAttribute("hook");
    let pciID = elem.getAttribute("customInteractionTypeIdentifier")
    //console.log(src)
    //console.log(pciID)
    if (src) {
      ...
      let RconfigSetup = document.createElement("script");
    
      RconfigSetup.innerHTML=` requirejs.config({
        baseUrl: 'js',
        paths: {
  ...
  let pciLauncher = document.createElement("script");
  
  pciLauncher.innerHTML=`
        requirejs(['jquery', 'pciObject'], function($, pciObject) {
          var event={addEventMgr(){console.log("FIXING EVENT MGR")}}
          //console.log(event)
          var $container = $(" .`+ pciID +`");
            //Récupérer le config pour le créer avant le lancement du initialize, il est le 3ème terme, puis assetmanager mais pas recommandé car pas QTI straight il me semble...
            //var config = `+ config +`;
            QTI.CUSTOM_INTERACTIONS = pciObject;
            pciObject.initialize("pciObject",$container,config);
            //$container.find(".prompt").html("Le prompt depuis le javascript") 
            $("properties").hide();  
            $container.find(".prompt").css("display","flex");        
        }); 
  `


      //Add here the AMD module and use the PCI launcher : pciname.initialize..
      //Check the modified dom using the inspector Element and not read source code. 

     //console.log(elem.markup)

      setTimeout(function() { //necessaire pour s'assurer du chargement de la base HTML à modifier
        //document.documentElement.append(scriptElem); //Modification.
        document.documentElement.append(RconfigSetup); //Modification.
        document.documentElement.append(pciLauncher); //Modification.
...
// "extension points", like portableCustomInteraction, although this code
...
function transform(elem) {
  ...
  function transformAttributes(elem, attribs) {
    ...
    [...elem.attributes].forEach(a => {
      ...
      } else if (elem.tagName=="portableCustomInteraction") {
  ...
  function transformChoice(elem, T) {
    ...
    default:
      // generic transform of choices inside a portableCustomInteraction. The
      // default handling of portableCustomInteractions is just to run the
    ...
    case "hottextInteraction":
      ...
      if (interaction.tagName=="choiceInteraction") {
        //console.log("Inside ChoiceInteraction"); //janfix
...
// Sets up a portableCustomInteraction (CI). If a setupInteraction function is
...
function setupportableCustomInteraction(htmlInteraction) {
  console.log(htmlInteraction)
  //console.log("SETUP CUSTOM INTERACTION USING htmlInteraction")
  //console.log(ID) // Erreur cela donne data-qtijs-identifier
  //let identifier = htmlInteraction.getAttribute('custom-Interaction-Type-Identifier');
  //console.log(identifier) //OK
  //let type = htmlInteraction.getAttribute("data-qtijs-tag");
  //console.log(type) //OK
  //let cisrc = htmlInteraction.getAttribute("hook");
 /*  let qtiInteraction = QTI.DOM.getElementById(htmlInteraction.id);
    console.log(qtiInteraction) */

 console.log($('div[data-qtijs-identifier="GGBPCI"]'))

 /*  $('div[data-qtijs-identifier="GGBPCI"]').find(".qtijs-next").on("click", function(){

    let responseVariable =QTI.CUSTOM_INTERACTIONS.getResponse();    
    console.log(responseVariable.base.integer)
    postResponseVariable(htmlInteraction,responseVariable.base.integer)
  })
 */

 
   $('div[data-qtijs-tag="portableCustomInteraction"]').on("mouseleave", function(){
    console.log("------------------------------------------")
    console.log(QTI.CUSTOM_INTERACTIONS)
    //console.log(htmlInteraction)
    let responseVariable =QTI.CUSTOM_INTERACTIONS.getResponse();    
    console.log(responseVariable.base.integer)
    postResponseVariable(htmlInteraction,responseVariable.base.integer)
  ...
  //OLD****************************************
  /* console.log(cisrc)//OK
  //ci doit être un objet chargé selon son identifiant ou son type
  let ci = QTI.CUSTOM_INTERACTIONS[identifier]//|| QTI.CUSTOM_INTERACTIONS[type];
  console.log(ci)
...
function validateAssessmentItem(item) {
  let interactions = {
    ...
    portableCustomInteraction:            validateportableCustomInteraction,
  ...
  function validateportableCustomInteraction(interaction) {
    return [];
  }
...
function getResponseVariable(htmlInteraction, variable) {
  ...
  console.log(item.declarations[variable])
